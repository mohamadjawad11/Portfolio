"use client";
import {BsArrowDownRight} from 'react-icons/bs'
import Link from 'next/link'

const services = [
  {
    num: "01",
    title: "Web Development",
    description: "Building responsive and modern websites using latest web technologies with expertise in ReactJs.",
    href:""
  },
  {
    num: "02",
    title: "Mobile Development",
    description: "Creating fast and user-friendly mobile apps for iOS and Android with expertise in React Native.",
    href:""
  },
  {
    num: "03",
    title: "Backend Development",
    description: "Designing robust server-side applications and APIs for scalable systems.",
    href:""
  },
  {
    num: "04",
    title: "Machine Learning",
    description: "Implementing intelligent models to analyze data and make predictions.",
    href:""
  },
];

import { motion } from 'framer-motion';

const Services = () => {
  return (
    <section className='min-h-[80vh] flex flex-col justify-center py-12 xl:py-0'>
      <div className='container mx-auto max-w-6xl pt-25'> {/* Modified line */}
        <motion.div initial={{opacity:0}} animate={{opacity:1,transition:{
          delay:2.4,
          duration:0.4,
          ease:"easeIn"
        }}}
        className='grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-[60px]'>

        {services.map((service, index)=>{ 
          return (
            <div key={index} className='flex-1 flex flex-col justify-center gap-6 group cursor-pointer ml-10 mr-10 xl:ml-0 xl:mr-0'>
              <div className='w-full flex justify-between items-center'>
                <div className='text-5xl font-extrabold text-outline text-transparent group-hover:text-outline-hover'>{service.num}</div>
                <Link href={service.href} className='w-[70px] h-[70px] rounded-full bg-white group-hover:bg-accent-deafult transition-all duration-500 flex justify-center items-center hover:-rotate-45'>
                  <BsArrowDownRight className='text-primary-color text-3xl'/>
                </Link>
              </div>
              <h2 className='text-[42px] font-bold leading-none text-white group-hover:text-accent-deafult transition-all duration-500'>{service.title}</h2>
              <p className='text-white-60'>{service.description}</p>
              <div className='border-b border-white/20 w-full'></div>
            </div>
          );
        })}

        </motion.div>
      </div>
    </section>
  );
}

export default Services;